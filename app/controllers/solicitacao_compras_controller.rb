class SolicitacaoComprasController < ApplicationController
  ...
  before_action :set_solicitacao_compra, only: %i[
                                           show edit update destroy autorizar confirm_destroy
                                           marcar_comprado cancelar_compra
                                         ]
...
if params[:forma_pagamento].present? && params[:forma_pagamento] != "todas"
  scope = scope.where(forma_pagamento: params[:forma_pagamento])
end
...
@solicitacao_compras = scope
  .includes(:itens)
  .order(Arel.sql("COALESCE(data_solicitacao, created_at::date) #{direction}, id #{direction}"))
  ...
  def autorizar
    ...
    nome_autorizador = if current_user&.participant
    ...
    if @solicitacao_compra.update(
      ...
      # 🔔 Notifica compradores (só depois de aprovada)
      notificar_compradores_aprovacao(@solicitacao_compra)
  ...
  def marcar_comprado
    unless can_current_user_buy?(@solicitacao_compra)
      redirect_to solicitacao_compras_path,
                  alert: "Você não tem permissão para marcar a compra."
      return
    end

    if @solicitacao_compra.update(status_compra: :comprado, motivo_cancelamento_compra: nil)
      # 🔔 Notifica o solicitante que a compra foi realizada
      notificar_solicitante_compra_realizada(@solicitacao_compra)

      redirect_to solicitacao_compras_path,
                  notice: "Compra marcada como realizada."
    else
      redirect_to solicitacao_compras_path,
                  alert: "Não foi possível atualizar o status da compra."
    end
  end
  ...
  def cancelar_compra
    unless can_current_user_buy?(@solicitacao_compra)
      redirect_to solicitacao_compras_path,
                  alert: "Você não tem permissão para cancelar a compra."
      return
    end

    motivo = params[:motivo_cancelamento_compra].presence
    ...
    if @solicitacao_compra.update(
      ...
      motivo_cancelamento_compra: motivo,
    )

      # 🔔 Notifica o solicitante que a compra foi cancelada
      notificar_solicitante_compra_cancelada(@solicitacao_compra, motivo)
  ...
  def notificar_solicitante_compra_cancelada(solicitacao, motivo)
    ...
    Notification.create!(
      ...
      titulo: "Sua solicitação de compra foi cancelada",
      corpo: "A solicitação de compra (##{solicitacao.id}) foi cancelada. Motivo: #{motivo}.",
      lida: false,
      notificavel: solicitacao, # se Notification tiver associação polymorphic
    )
  end

  def notificar_solicitante_compra_realizada(solicitacao)
    # 🔎 Descobre o participant que é o COLABORADOR da solicitação
    solicitante = Participant.find_by(name: solicitacao.colaborador)
    return unless solicitante&.user # precisa ter user associado
    ...
    Notification.create!(
      user: usuario_solicitante,
      titulo: "Sua solicitação de compra foi realizada",
      corpo: "A solicitação de compra (##{solicitacao.id}) foi marcada como comprada. Em breve o item/serviço estará disponível.",
      lida: false,
      notificavel: solicitacao, # se Notification tiver associação polymorphic
    )
  end

  def notificar_compradores_aprovacao(solicitacao)
    # 🔎 Descobre o participant que é o COLABORADOR da solicitação
    solicitante = Participant.find_by(name: solicitacao.colaborador)
    ...
    # ✅ Busca TODOS os participants da MESMA EMPRESA que podem COMPRAR (can_buy = true)
    compradores_participants = Participant
    ...
    compradores_users = compradores_participants.map(&:user).compact.uniq

    return if compradores_users.empty?

    compradores_users.each do |user|
      Notification.create!(
        user: user,
        titulo: "Nova solicitação de compra aprovada",
        corpo: "Há uma solicitação de compra aprovada para o colaborador #{solicitacao.colaborador}. Verifique para dar andamento à compra.",
        lida: false,
        notificavel: solicitacao, # se Notification tiver associação polymorphic
      )
    end
  ...
  # Pode ver tudo se for autorizador OU comprador
  sub.can_approve_purchase? || sub.can_buy?
end


  def solicitante_apenas?
    participant = current_user&.participant
    sub = participant&.sub_grupo_empresa
    return false unless sub

    # Pode SOLICITAR, mas NÃO autoriza e NÃO compra
    sub.can_request_purchase? && !sub.can_approve_purchase? && !sub.can_buy?
  end
  ...
  # 🔓 Admin/operador pode editar SEM restrição
  return if admin?

  # 🔒 Para os demais: uma vez autorizada, ninguém edita mais
  ...
  # 🔓 Admin/operador pode excluir SEM restrição
  return if admin?

  # 🔒 Regra para os demais:
  # - Autorizada + compra NÃO cancelada  -> NÃO pode excluir
  # - Autorizada + compra cancelada      -> PODE excluir
  return unless @solicitacao_compra.autorizado?

  # se a compra foi cancelada, libera a exclusão
  return if @solicitacao_compra.cancelado? # enum status_compra: :aguardando, :comprado, :cancelado

  redirect_to solicitacao_compras_path,
              alert: "Esta solicitação já foi autorizada e não pode ser excluída. Cancele a compra para poder excluir."
end


  def ensure_can_view
  return if admin?
  return if can_see_all_solicitacoes?
  ...
  def can_current_user_buy?(_solicitacao)
    participant = current_user&.participant
    sub = participant&.sub_grupo_empresa

    # Só pode se tiver subgrupo e o subgrupo for comprador
    sub&.can_buy?
  end

  helper_method :can_current_user_buy?
